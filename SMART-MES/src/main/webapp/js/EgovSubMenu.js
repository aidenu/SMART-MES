...
var imgpath         = "/tmanager/images/egovframework/com/cmm/utl/";
var treeYeobu       = false;
var chkValue        = "";
var vHtmlCode       = "";
...
* 트리생성함수
*/
function createTree(arrName, vYeobu, checkValue,currentMenuNo) {
   var startNode, openNode;
	...
	if (treeNodes.length > 0) {
		...
		addTreeNode(startNode, recursedNodes,currentMenuNo);
...
* 신규 트리노드 추가
*/
function addTreeNode(parentNode, recursedNodes,currentMenuNo) {
		...
		if (nodeValues[1] == parentNode) {
			...
			if (hasChildNode) {
				...
				if(recursedNodes.length==1){
					//현재 메뉴이면 빨간색 표시
					//alert(currentUrl + "::" + nodeValues[5].substring(0,nodeValues[5].lastIndexOf(".")));
					//if(currentUrl ==nodeValues[5].substring(0,nodeValues[5].lastIndexOf("."))){
					if(currentMenuNo ==nodeValues[0]){
						vHtmlCode +=vHtmlCodeBg+"<a id="+i+" href=javascript:fn_MovePage('" + i + "');><b><font size=3 color='#F6FBAF'>"+nodeValues[2]+"</font></b></a></li>";
					}else{
						vHtmlCode +=vHtmlCodeBg+"<a id="+i+" href=javascript:fn_MovePage('" + i + "');><font size=3 color='#E1EFFF'>"+nodeValues[2]+"</font></a></li>";
					}
				   
				}else{
				   vHtmlCode +=vHtmlCodeBgList+"<a id="+i+" href=javascript:fn_MovePage('" + i + "');><font size=3 color='#E1EFFF'>"+nodeValues[2]+"</font></a></li>";
				}
			}
