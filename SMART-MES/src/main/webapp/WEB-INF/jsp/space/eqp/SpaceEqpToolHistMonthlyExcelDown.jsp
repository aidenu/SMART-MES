...
<%
    response.setHeader("Content-Disposition", "attachment; filename=MonthlyStockData.xls"); 
...
<body>
	<table border='1'>
		<tr>
			<td bgcolor='#D9D900' align='center' style='mso-number-format:"\@";'>${searchDate }</td>
		</tr>
		<tr>
			<td>
				<table border='1'>
					<tr>
						<td align='center' bgcolor='#EDB752' rowspan="2">대분류</th>
						<td align='center' bgcolor='#EDB752' rowspan="2">소분류</th>
						<td align='center' bgcolor='#EDB752' rowspan="2">파이(mm)</th>
						<td align='center' bgcolor='#EDB752' rowspan="2">F/B</th>
						<td align='center' bgcolor='#EDB752' rowspan="2">R(mm)</th>
						<td align='center' bgcolor='#EDB752' rowspan="2">기장(mm)</th>
					</tr>
				</table>
			</td>
			<td>
				<table border='1'>
					<tr>
						<c:forEach var="listDate" items="${listDate }" varStatus="dateStatus">
							<td align='center' bgcolor='#EDB752' colspan="3">${listDate }일</th>	
						</c:forEach>
					...
					<tr>
						<c:forEach var="listDate" items="${listDate }" varStatus="dateStatus">
							...
							<td align='center' bgcolor='#EDB752'>재고</th>
						</c:forEach>
						<td align='center' bgcolor='#EDB752'>입고</th>
						<td align='center' bgcolor='#EDB752'>사용</th>
					</tr>
			...
			<td>
				...
				<c:forEach var="result" items="${result }" varStatus="status">
					...
					<c:forEach var="listDate" items="${listDate }" varStatus="statuslist">
						...
						<td align='center'>${result[remainDate] }</td>
					</c:forEach>
