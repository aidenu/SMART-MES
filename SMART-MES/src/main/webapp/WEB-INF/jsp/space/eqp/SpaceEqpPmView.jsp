<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>

<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="ui" uri="http://egovframework.gov/ctl/ui"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>

<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Language" content="ko" >
<meta http-equiv="Content-Language" content="ko" >
<title>설비PM</title>
<link rel="shortcut icon" href="<c:url value='/'/>images/bl_circle.gif">
<link href="<c:url value='/css/common.css'/>" rel="stylesheet" type="text/css" >
<link href="<c:url value='/css/space/table.css'/>" rel="stylesheet" type="text/css" >
<link href="<c:url value='/css/space/AXButton.css'/>" rel="stylesheet" type="text/css" >
<link href="<c:url value='/css/jquery/calendarview.css'/>" rel="stylesheet" type="text/css" >
<link href="<c:url value='/css/titlefix/defaultTheme.css'/>" rel="stylesheet" type="text/css" >
<link href="<c:url value='/css/titlefix/myTheme.css'/>" rel="stylesheet" type="text/css" >

<script type="text/javascript" src="<c:url value="/js/jquery/jquery.min.js"/>"/></script>
<script type="text/javascript">var jq$ =jQuery.noConflict();</script>
<script type="text/javascript" src="<c:url value="/js/jquery/prototype.js"/>"/></script>
<script type="text/javascript" src="<c:url value="/js/jquery/calendarview.js"/>"/></script>
<script type="text/javascript" src="<c:url value="/js/titlefix/jquery.fixedheadertable.js"/>"/></script>
<script type="text/javascript" src="<c:url value="/js/spacevalidate.js"/>"/></script>
<script type="text/javascript" src="<c:url value="/js/spaceutil.js"/>"/></script>

<script>
	
	window.onload = function() {
		
		// 날짜 입력 항목 초기화
		Calendar.setup(
		          {
		            dateField: 'pm_date',
		            triggerElement: 'pm_date'
		          }
		)
		
		getEqpList('FIRST');
	}
	
	
	
	function setInitEqp()
	{
		if(document.dataForm.eqpname.options.length > 1)
		{
			for(var i=0 ; i<document.dataForm.eqpname.options.length ; i++)
			{
				if(document.dataForm.eqpname.options[i].value == "${PM_EQP}")
				{
					document.dataForm.eqpname.options[i].selected = true;
				}
			}
		}
	}


	function getEqpList(firstflag)
	{
		document.dataForm.action="${pageContext.request.contextPath}/space/eqp/SpaceEqpPmEqpList.do?gubun=INSERT&firstflag="+firstflag;
		document.dataForm.target = "searchFrame";
		document.dataForm.submit();
	}
	
	
	
	function setData(actiontype)
	{
		if(actiontype == "DELETE")
		{
			if(confirm("설비PM을 삭제 하시겠습니까?"))
			{
				document.getElementById("progress_div").style.visibility = "visible";
				document.dataForm.action="${pageContext.request.contextPath}/space/eqp/SpaceEqpPmSave.do?actiontype=DELETE";
				document.dataForm.target = "searchFrame";
				document.dataForm.submit();
			}
		}
		else
		{
			if(document.dataForm.pm_type.value == "")
			{
				alert("구분을 선택해 주세요");
				document.dataForm.pm_type.focus();
				return;
			}
			
			if(document.dataForm.eqppart.value == "")
			{
				alert("설비그룹을 선택해 주세요");
				document.dataForm.eqppart.focus();
				return;
			}
			
			if(document.dataForm.eqpname.value == "")
			{
				alert("설비명을 선택해 주세요");
				document.dataForm.eqpname.focus();
				return;
			}
			
			
			if(document.dataForm.pm_detail.value == "")
			{
				alert("작업내용을 입력해 주세요");
				document.dataForm.pm_detail.focus();
				return;
			}
			
			if(confirm("입력하신 값으로 설비PM을 저장 하시겠습니까?"))
			{
				document.getElementById("progress_div").style.visibility = "visible";
				document.dataForm.action="${pageContext.request.contextPath}/space/eqp/SpaceEqpPmSave.do?actiontype=UPDATE";
				document.dataForm.target = "searchFrame";
				document.dataForm.submit();
			}
		}
	}
	
	
	
	function HandleBrowseClick(itemIndex)
	{
		document.getElementById('item_image_'+itemIndex).value = '';
	    var fileinput = document.getElementById("item_image_"+itemIndex);
	    fileinput.click();
	}
	
	
	function selectFolder(e,itemIndex)
	{
		var theFiles = e.target.files;
		var filename = "";
		var imageType = /image.*/;
		
		//폴더가 선택되면 기존 이미지는 지워준다
		document.getElementById("preview_"+itemIndex).src = "";
				
		for (var i=0, file; file=theFiles[i]; i++)
		{
			filename = file.name;
		
			if(file.size > 1000000)//이미지 사이즈는 1M 이하만 등록 가능
			{
				alert("이미지 사이즈는 1M 이하만 등록 가능 합니다.");
				return false;
				break;
			}
		
			if(file.type.match(imageType))//선택된 폴더에서 이미지 파일을 화면에 보여준다 
			{
				var imagereader = new FileReader();
				
				imagereader.onload = function(file){
					document.getElementById("preview_"+itemIndex).src = this.result;
					document.getElementById("preview_"+itemIndex).width = 150;
					document.getElementById("preview_"+itemIndex).height = 150;
				};
		
				imagereader.readAsDataURL(file);
			}
			else
			{
				alert("등록 가능한 이미지 파일은 gif,jpg,jpeg,bmp,png 파일 입니다");
				document.getElementById("preview_"+itemIndex).src = "<c:url value='/'/>images/png/no_image_item.png";
				return false;
				break;
			}
		}
	} 
	
	
	
	function getImageView(itemIndex)
	{
		document.imageForm.image.value = document.getElementById("preview_" + itemIndex).src;
		
		// 팝업 창을 post 방식으로 열기 위함.
		var myForm = document.imageForm;
		 var url = "<c:url value='/'/>space/common/SpaceImageView.do";
		 window.open("" ,"imageForm", 
		       "scrollbars=yes,toolbar=no,resizable=yes,left=200,top=200,width=500,height=500"); 
		 myForm.action =url; 
		 myForm.method="post";
		 myForm.target="imageForm";
		 myForm.submit();
	}
	
</script>
</head>

<body style="background-color: #2c3338;">
<form name="dataForm" method="post" enctype="multipart/form-data">
<input type="hidden" name="pm_id" value="${PM_ID}">
<div id="wrap">
	<div id="spacecontainer">
		<table>
			<tr><td colspan="3">&nbsp;</td></tr>
			<tr>
				<td  valign="top">
					<table>
						<tr>
							<td>
								<div style="width:200px;border: 1px solid #D5D1B0;">
									<table width="100%">
										<tr>
											<td id="captionSubTitle" align="center">설비PM</td>
										</tr>
									</table>
								</div>
							</td>
						</tr>
						<tr>
							<td>
								<div class="tableContainer" style="width:670px;">
									
									<table width="100%">
										<tr>
											<td width="10%" id="captionTitle">구분</td>
											<td width="20%" id="captionSubTitle" align="left" style="padding-left:10px">
												<select name="pm_type" id="pm_type" style="width:80px">
													<option value="">-선택-</option>
													<option value="CHECK" <c:if test="${PM_TYPE == 'CHECK'}">selected</c:if>>정기점검</option>
													<option value="REPAIR" <c:if test="${PM_TYPE == 'REPAIR'}">selected</c:if>>고장/수리</option>
												</select>
											</td>
											<td width="10%" id="captionTitle">설비그룹</td>
											<td width="20%" id="captionSubTitle" align="left" style="padding-left:10px">
												<select name="eqppart" id="eqppart" onchange="getEqpList()" style="width:100px">
													<option value="">-선택-</option>
													<c:forEach var="resultBasic" items="${resultBasic}" varStatus="status">
														<option value="${resultBasic.EQP_PART}" <c:if test="${resultBasic.EQP_PART == PM_EQPPART}">selected</c:if>>${resultBasic.EQP_PART}</option>
													</c:forEach>
												</select>
											</td>
											<td width="10%" id="captionTitle">설비명</td>
											<td width="30%" id="captionSubTitle" align="left" style="padding-left:10px">
												<select name="eqpname" id="eqpname" style="width:200px">
													<option value="">-선택-</option>
												</select>
											</td>
										</tr>
										<tr>
											<td id="captionTitle">작업<br>일자</td>
											<td id="captionSubTitle" align="left" style="padding-left:10px">
												<input type="text" name="pm_date" id="pm_date" style="height:17px;width:80px;text-align:center;cursor:pointer" value="${PM_DATE}" readonly>
											</td>
											<td id="captionTitle">작업자</td>
											<td id="captionSubTitle" align="left" style="padding-left:10px">
												${PM_WORKER_NAME}
											</td>
											<td id="captionTitle">작업<br>시간</td>
											<td id="captionSubTitle" align="left" style="padding-left:10px">
												<input type="text" name="pm_hour" style="height:17px;width:80px;text-align:center;" value="${PM_HOUR}" onkeyup="onlyNum(this);"> 시간
											</td>
										</tr>
										<tr>
											<td id="captionTitle">작업<br>비용</td>
											<td id="captionSubTitle" align="left" style="padding-left:10px" colspan="5">
												<input type="text" name="pm_cost" style="height:17px;width:80px;text-align:center;" value="${PM_COST}" onkeyup="onlyNum(this);this.value=this.value.comma();"> 원
											</td>
										</tr>
										<tr>
											<td id="captionTitle">작업<br>내용</td>
											<td id="captionSubTitle" align="left" style="padding-left:10px" colspan="5">
												<textarea name="pm_detail" id="pm_detail" style="height:47px;width:590px;" onkeyup="checkStrLength(500,this);checkQuota(this)">${PM_DETAIL}</textarea>
											</td>
										</tr>
										<tr>
											<td id="captionTitle">비고</td>
											<td id="captionSubTitle" align="left" style="padding-left:10px" colspan="5">
												<textarea name="pm_desc" id="pm_desc" style="height:47px;width:590px;" onkeyup="checkStrLength(500,this);checkQuota(this)">${PM_DESC}</textarea>
											</td>
										</tr>
									</table>
									<br>
									<table width="100%">
										<tr>
											<td width="50%" id="captionTitle" colspan="2">작업전</td>
											<td width="50%" id="captionTitle" colspan="2">작업후</td>
										</tr>
										<tr>
											<td width="25%" id="captionSubTitle" align="center" style="width:200px;height:200px;text-align:center">
												<input type="file" name="file_image_1" id="item_image_1" style="display:none" onChange="selectFolder(event,1)">
												<c:if test="${PM_WORKER == userid || userrole == 'ROLE_ADMIN' || userrole == 'ROLE_USER_MANAGE'}"><a href="#" class="AXButtonSmall Red" onclick="HandleBrowseClick(1)">이미지선택</a></c:if>
												<br><br>
												<c:choose>
													<c:when test="${IMAGE_INFO_1 eq null}">
														<img	id="preview_1" src="<c:url value='/'/>images/png/no_image_item.png" />
													</c:when>
													<c:otherwise>
														<img	id='preview_1' src='<c:url value='/'/>space/common/SpaceFileDown.do?filelocation=${FILE_LOCATION}&filename=${IMAGE_INFO_1}' width='150' height='150' style="cursor:pointer" onclick="getImageView(1)" />
													</c:otherwise>
												</c:choose>
											</td>
											<td width="25%" id="captionSubTitle" align="center" style="width:200px;height:200px;text-align:center">
												<input type="file" name="file_image_2" id="item_image_2" style="display:none" onChange="selectFolder(event,2)">
												<c:if test="${PM_WORKER == userid || userrole == 'ROLE_ADMIN' || userrole == 'ROLE_USER_MANAGE'}"><a href="#" class="AXButtonSmall Red" onclick="HandleBrowseClick(2)">이미지선택</a></c:if>
												<br><br>
												<c:choose>
													<c:when test="${IMAGE_INFO_2 eq null}">
														<img	id="preview_2" src="<c:url value='/'/>images/png/no_image_item.png" />
													</c:when>
													<c:otherwise>
														<img	id='preview_2' src='<c:url value='/'/>space/common/SpaceFileDown.do?filelocation=${FILE_LOCATION}&filename=${IMAGE_INFO_2}' width='150' height='150' style="cursor:pointer" onclick="getImageView(2)" />
													</c:otherwise>
												</c:choose>
											</td>
											<td width="25%" id="captionSubTitle" align="center" style="width:200px;height:200px;text-align:center">
												<input type="file" name="file_image_3" id="item_image_3" style="display:none" onChange="selectFolder(event,3)">
												<c:if test="${PM_WORKER == userid || userrole == 'ROLE_ADMIN' || userrole == 'ROLE_USER_MANAGE'}"><a href="#" class="AXButtonSmall Red" onclick="HandleBrowseClick(3)">이미지선택</a></c:if>
												<br><br>
												<c:choose>
													<c:when test="${IMAGE_INFO_3 eq null}">
														<img	id="preview_3" src="<c:url value='/'/>images/png/no_image_item.png" />
													</c:when>
													<c:otherwise>
														<img	id='preview_3' src='<c:url value='/'/>space/common/SpaceFileDown.do?filelocation=${FILE_LOCATION}&filename=${IMAGE_INFO_3}' width='150' height='150' style="cursor:pointer" onclick="getImageView(3)" />
													</c:otherwise>
												</c:choose>
											</td>
											<td width="25%" id="captionSubTitle" align="center" style="width:200px;height:200px;text-align:center">
												<input type="file" name="file_image_4" id="item_image_4" style="display:none" onChange="selectFolder(event,4)">
												<c:if test="${PM_WORKER == userid || userrole == 'ROLE_ADMIN' || userrole == 'ROLE_USER_MANAGE'}"><a href="#" class="AXButtonSmall Red" onclick="HandleBrowseClick(4)">이미지선택</a></c:if>
												<br><br>
												<c:choose>
													<c:when test="${IMAGE_INFO_4 eq null}">
														<img	id="preview_4" src="<c:url value='/'/>images/png/no_image_item.png" />
													</c:when>
													<c:otherwise>
														<img	id='preview_4' src='<c:url value='/'/>space/common/SpaceFileDown.do?filelocation=${FILE_LOCATION}&filename=${IMAGE_INFO_4}' width='150' height='150' style="cursor:pointer" onclick="getImageView(4)" />
													</c:otherwise>
												</c:choose>
											</td>
										</tr>
									</table>
								</div>
							</td>
						</tr>
						
						<tr><td>&nbsp;</td></tr>
						<tr>
							<td id='btn_layer' align="center">
								<c:if test="${PM_WORKER == userid || userrole == 'ROLE_ADMIN' || userrole == 'ROLE_USER_MANAGE'}">
									<a href='#' class='AXButton Gray' onclick="setData('UPDATE')">저장</a>
									&nbsp;
									<a href='#' class='AXButton Gray' onclick="setData('DELETE')">삭제</a>
									&nbsp;
								</c:if>
								<a href='#' class='AXButton Gray' onclick="self.close()">닫기</a>
							</td>
						</tr>
					</table>
				</td>
			</tr>
		</table>
	</div>
</div>
<!-- //전체 레이어 끝 -->
</form>

<div id="progress_div" style="position:absolute;width:100%;height:100%;top:0;left:0;background-color:#DCDCED;opacity:0.5;visibility:hidden;z-index:1">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%">
		<tr>
			<td align=center><img src="<c:url value='/'/>images/space/progressdisc.gif" width="30" height="30" border="0"></td>
		</tr>
	</table>
</div>

<form name="imageForm" method="post"><input type="hidden" name="image"></form>
<iframe name="searchFrame" width="0" hieght="0" style="display:none"></iframe>
</body>
</html>